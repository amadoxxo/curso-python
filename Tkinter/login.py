...
class LoginVentana(tk.Tk):
    ...
    def _crear_componentes(self):
        ...
        usuario_etiqueta.grid(column=0, row=0, sticky=tk.E ,padx=5, pady=5)
        self.usuario_entrada = ttk.Entry(self)
        self.usuario_entrada.grid(column=1, row=0, sticky=tk.W, padx=5, pady=5)

        # password
        password_etiqueta = ttk.Label(self, text='Password:')
        password_etiqueta.grid(column=0, row=1, sticky=tk.E, padx=5, pady=5)
        self.password_entrada = ttk.Entry(self ,show='*')
        self.password_entrada.grid(column=1, row=1, sticky=tk.W, padx=5, pady=5)

        # boton Login
        login_boton = ttk.Button(self, text='Login', command=self._login)
        login_boton.grid(column=0, row=3, columnspan=2)
